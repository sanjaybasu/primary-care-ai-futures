...
implements causal world model with state encoder, transition module, and outcome module

data from nhanes mortality linkage, state-level integrated data, and county-level svi
"""
...
# nhanes doesn't include state identifiers in public data
...
state_pop['pop_weight'] = 1 / len(state_pop)
...
# use temporal variation as primary source of intervention exposure
...
if PYTORCH_AVAILABLE:
    ...
    class WorldModel(nn.Module):
        """
        neural network world model with encoder, transition, and outcome modules
        """
...
def doubly_robust_estimate(X, y, treatment_col_idx, model, scaler, bootstrap_n=1000):
    """
    doubly robust estimate for intervention effect via aipw
    """
...
for i, col in enumerate(intervention_cols[:8]):  # Skip AI interventions (no exposure in data)
    ...
    # hr approx rr for rare outcomes
    hr = rr
    hr_lower = ci_lower
    hr_upper = ci_upper
